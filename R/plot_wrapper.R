...
plt1536 <- function(platemap){
    ggplot(data = platemap, aes(x = Column, y = Row)) +
	geom_point(data = expand.grid(seq(1, 48), seq(1, 32)), aes(x = Var1, y = Var2),
		   color = "grey90", fill = "white", shape = 22, size = 2) + 
	geom_point(aes(fill = values), colour = "gray20", shape = 22, size = 3.5) + 

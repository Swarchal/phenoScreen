...
# well_to_num
#------------------------------------------------------------------------------
# converts well ID into well number for 96-well plates. 
#   e.g B01 -> 13, H12 -> 96
# takes a vector of well ID's and outputs a vector of well numbers in the same
# order.
#
# argument `style` is for choosing plate patterns. The default style is a row
# by row basis. The other option is `snake`, in which at the end of the row,
# the next row is reversed
#   e.g ...A11, A12, B12, B11 ...
###############################################################################

well_to_num <- function(wells, style = "normal"){
...
#         class = "data.frame", row.names = c(NA, -96L))
    ...
    if(style == "snake"){
        return(which(well_list_snake$well %in% wells))
    }
    
    if(style == "normal"){
        return(which(well_list$well %in% wells))
    }
    valid <- c("snake", "normal")
    if(style != valid){print("not a valid plate format")}
}